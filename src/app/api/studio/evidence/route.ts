...
const EVENT_ACTIVITY_LABELS: Record<string, string> = {
...
const EVENT_METHOD_LABELS: Record<string, string> = {
  phone_screen: 'Phone',
  technical:    'Video / Phone',
  behavioral:   'Video / Phone',
...
export async function GET(req: NextRequest) {
  ...
  if (!from || !to) return Response.json({ error: 'from and to query params are required' }, { status: 400 })

  const ownerId = session.user.id

  const appRows = await sql`
    SELECT
      applied_at::text AS date,
      company,
      role,
      'Application Submitted' AS activity,
      'Online Portal' AS method,
  ...
  const eventRows = await sql`
    SELECT
      COALESCE(e.occurred_at::text, e.created_at::date::text) AS date,
      a.company,
      a.role,
      e.event_type AS raw_activity,
  ...
  const events = eventRows.map((r) => ({
    ...
    activity: EVENT_ACTIVITY_LABELS[r.raw_activity as string] ?? String(r.raw_activity),
    method:   EVENT_METHOD_LABELS[r.raw_activity as string]   ?? 'Various',
