...
// Auth.js's provider init. Bypass it by switching to type:"oauth" with
// hardcoded endpoints. The /v2/userinfo shape (sub, name, email, picture)
// matches the OIDC claims so the jwt callback is unchanged.
// clientId/clientSecret must be set directly on the object — Auth.js's
// env-var injection only runs on provider function references, not on
// spread objects, so without this the token exchange fails (error=Configuration).
...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  callbacks: {
    async jwt({ token, user, account, profile }) {
      // LinkedIn OAuth — first sign-in: account + user populated by profile()
